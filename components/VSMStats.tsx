       ...
       <div className="space-y-3">
          ...
          <div className="flex flex-col group/pce relative">
             ...
             <div className="absolute bottom-full mb-2 left-0 w-full bg-slate-800 text-white text-[10px] p-2 rounded opacity-0 group-hover/pce:opacity-100 transition-opacity pointer-events-none z-50">
                 <div className="flex justify-between"><span>Value Added:</span> <span>{avgVAT.toFixed(1)}t</span></div>
                 <div className="flex justify-between"><span>Transit:</span> <span>{avgTransit.toFixed(1)}t</span></div>
                 <div className="flex justify-between"><span>Waiting:</span> <span>{(avgLeadTime - avgVAT - avgTransit).toFixed(1)}t</span></div>
             </div>
          </div>

          <div className="grid grid-cols-2 gap-3 pt-2">
             {/* Lead Time */}
             <div className="bg-slate-50 p-2 rounded-lg border border-slate-100" title={`Avg Transit: ${avgTransit.toFixed(1)}t`}>
                <div className="text-[10px] text-slate-400 uppercase font-bold flex items-center gap-1">
                   <Clock size={10} /> Lead Time
                </div>
                <div className="text-lg font-mono font-bold text-slate-700">
                   {avgLeadTime.toFixed(0)}<span className="text-xs font-sans font-normal text-slate-400">t</span>
                </div>
             </div>
          ...
          <div className="text-xs text-center text-slate-400 pt-1 border-t border-slate-100 mt-2 flex items-center justify-center gap-2">
             <Activity size={12} /> Throughput: <strong className="text-slate-600">{currentThroughput.toFixed(1)}</strong> / 100t
          </div>
       </div>
    </div>
  );
